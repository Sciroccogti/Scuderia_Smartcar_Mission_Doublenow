...
int iniLeaveSpeedCount; // 出环岛减速时间常量
...
// 下坡判定电感值
int16 DownAD0 = 4000, DownAD1 = 4000, DownAD2 = 2000, DownAD3 = 2000;
...
void DirectionControl(void) {
        ...
        !FreezingTime)  //到达电感阈值且不与上一次判定重复
    {
        ...
        {
            // if(g_ValueOfAD[2] >= g_ValueOfAD[3]){

            // }
            round_delay = round_delay_distance;
        }
        ...
        {
            Leave = OFF;
            LeaveSpeedCount = iniLeaveSpeedCount;
            // gpio_set(D13, 0);
...
void Read_ADC(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        ad_valu[1][i] =
            adc_mean_filter(ADC_1, AD5, 5) * 10000 / 10000;  // 水平右
