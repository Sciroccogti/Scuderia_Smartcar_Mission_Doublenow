...
int32 LeftDead = 0;
int32 RighDead = 0;
float Ratio_Encoder_Left = 200/(1175*0.02);// 左轮速度=counter*左轮周长(mm)/(左轮转一圈对应的脉冲数*程序周期)
float Ratio_Encoder_Righ = 200/(1175*0.02);// 右轮速度=counter*右轮周长(mm)/(右轮转一圈对应的脉冲数*程序周期)
...
void CalSpeedError(void)
{
	...
	/*//KEA version
	g_nLeftpulse = (GPIO_Get(E1) ==1?FTM_Pulse_Get(ftm0):-FTM_Pulse_Get(ftm0));//读取左轮脉冲
	FTM_Count_Clean(ftm0);
	g_nRighpulse = (GPIO_Get(H6)==0?FTM_Pulse_Get(ftm1):-FTM_Pulse_Get(ftm1));//读取右轮脉冲
