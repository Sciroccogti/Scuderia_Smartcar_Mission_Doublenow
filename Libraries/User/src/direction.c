...
/**********全局变量定义********/ 
float g_fDirectionError[2];		//方向偏差（g_fDirectionError[0]为一对水平电感的差比和偏差）								//		  （g_fDirectionError[1]为一对垂直电感的差比和偏差）
float g_fDirectionError_dot[2];//方向偏差微分（g_fDirectionError_dot[0]为一对水平电感的差比和偏差微分）								//			 （g_fDirectionError_dot[1]为一对垂直电感的差比和偏差微分）
...
int16 TurnAD0 = 2000,TurnAD1 = 2000,TurnAD2 = 1200,TurnAD3 = 1200;//水平左右，垂直左右电感，判断是否到达环岛的阈值
int16 LeaveAD0 = 1300,LeaveAD1 = 1300,LeaveAD2 = 1000,LeaveAD3 = 1000;//离开环岛的阈值放松
int16 DownAD0 = 2000,DownAD1 = 2000,DownAD2 = 1000,DownAD3 = 1000;//下坡判定电感值

float Environment = 0.9;//受环境影响的电感系数  把左右水平电感控制在 800左右（参考）
 ...
 *				(水平左电感)				g_ValueOfAD[2](水平中电感）				（水平右电感）
 *						
 * @date		2018
 */
void DirectionControl(void)
{
	...
	g_ValueOfAD[3] = (g_ValueOfAD[3] < 10? 10:g_ValueOfAD[3]);
        for(int ADi = 0;ADi<4;ADi++)
          g_ValueOfAD[ADi] *= Environment;
	...
	g_fDirectionError_dot[1] = (g_fDirectionError_dot[1]<-0.7?-0.7:g_fDirectionError_dot[1]);
          ...
          //以下为环岛处理
          if((g_ValueOfAD[0]>TurnAD0)&&(g_ValueOfAD[1]>TurnAD1)&&((g_ValueOfAD[2]>TurnAD2)||(g_ValueOfAD[3]>TurnAD3))&& !FreezingTime  )//到达电感阈值且不与上一次判定重复
          {
            ...
            if((g_ValueOfAD[0]>LeaveAD0)&&(g_ValueOfAD[1]>LeaveAD1)&&((g_ValueOfAD[2]>LeaveAD2)||(g_ValueOfAD[3]>LeaveAD3))&& !FreezingTime )
            {
              if(Leave == ON)//离岛标志，亮红灯，不改变方向控制，冻结一次进岛判定时间
              {
                Leave = OFF;
                gpio_set(H0,1);
                FreezingTime = FreezingTimeDuring;
        ...
        if(g_ValueOfAD[0]<DownAD0 && g_ValueOfAD[1]<DownAD1 && g_ValueOfAD[2]>DownAD2 && g_ValueOfAD[3]>DownAD3 && !DownTime  && !FreezingTime && (OFF==Leave))//下坡减速
        {
	...
	//方向算法（位置式PD）
        if(Flag_Round == ON)
	g_fDirectionControlOut = (g_fDirectionError[1]*Turn_dirControl_P + g_fDirectionError_dot[1]*Turn_dirControl_D);//依据垂直电感转向
        else
	g_fDirectionControlOut = (g_fDirectionError[0]*g_dirControl_P + g_fDirectionError_dot[0]*g_dirControl_D);//依据水平电感转向
